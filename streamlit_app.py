    ...
    if submit and model is not None:
        ...
        try:
            ...
            if size_input >= 2.0 or emr_in or rad_in:
                res, bg_color, advice = "🔴 High Risk", "#FF4B4B", "เฝ้าระวังเข้มงวด: โทรติดตามอาการวันที่ 1, 2 และ 3 Add Line"
                ...
                res, bg_color, advice = "🟢 Low Risk", "#28A745", "ให้คำแนะนำสังเกตอาการด้วยตนเอง Add Line"
                prob = np.random.uniform(0.015, 0.085)
            elif clip_in or med_in or hot_in or size_input >= 1.0 or prob_raw > 0.12:
                res, bg_color, advice, text_color = "🟡 Moderate Risk", "#FFFF00", "เฝ้าระวังต่อเนื่อง: โทรติดตามอาการในวันที่ 2 Add Line", "#000000"
            ...
            new_row = pd.DataFrame([{
                ...
                "Clip": int(clip_in), "Risk_Level": res, "Advice": advice, "AI_Score": prob
            }])
...
with tab2:
    st.header("📊 สรุปผลภาพรวม (Dashboard)")
    df = get_data()
    if not df.empty:
        df_plot = df.copy()
        ...
        m1.metric("เคสทั้งหมด", f"{len(df_plot)} ราย")
        m2.metric("High Risk", len(df_plot[df_plot['Risk_Summary'] == 'High Risk']))
        m3.metric("Moderate Risk", len(df_plot[df_plot['Risk_Summary'] == 'Moderate Risk']))
        m4.metric("Low Risk", len(df_plot[df_plot['Risk_Summary'] == 'Low Risk']))
        ...
        with c_left:
            fig_pie = px.pie(
                df_plot, names='Risk_Summary', color='Risk_Summary',
        ...
        with c_right:
            ...
            def highlight_risk(val):
                if 'High' in str(val): return 'background-color: #FF4B4B; color: white;'
                elif 'Moderate' in str(val): return 'background-color: #FFFF00; color: black;'
                elif 'Low' in str(val): return 'background-color: #28A745; color: white;'
                return ''
