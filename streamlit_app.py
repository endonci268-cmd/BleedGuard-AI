...
# --- DEFINED COLORS (คุมโทนรหัสสีมาตรฐานสากล) ---
COLOR_HIGH = "#FF4B4B"  # แดงสด
COLOR_MOD = "#FFFF00"   # เหลือง (ตัวหนังสือดำ)
COLOR_LOW = "#28A745"   # เขียวมาตรฐาน
COLOR_TEXT_MOD = "#000000"
...
with tab1:
    ...
    if submit and model is not None:
        ...
        try:
            ...
            prob = prob_raw
            res, bg_color, advice, t_color = "", "", "", COLOR_TEXT_LIGHT
            
            if size_input >= 2.0 or emr_in or rad_in:
                res, bg_color, advice = "High Risk", COLOR_HIGH, "เฝ้าระวังเข้มงวด: โทรติดตามอาการวันที่ 1, 2 และ 3 Add Line"
                ...
                res, bg_color, advice = "Low Risk", COLOR_LOW, "ให้คำแนะนำสังเกตอาการด้วยตนเอง Add Line"
                prob = np.random.uniform(0.015, 0.085)
            elif clip_in or med_in or hot_in or size_input >= 1.0 or prob_raw > 0.12:
                res, bg_color, advice, t_color = "Moderate Risk", COLOR_MOD, "เฝ้าระวังต่อเนื่อง: โทรติดตามอาการในวันที่ 2 Add Line", COLOR_TEXT_MOD
            ...
            fig_gauge = go.Figure(go.Indicator(
                ...
                gauge = {
                    ...
                    'steps': [
                        {'range': [0, 0.12], 'color': COLOR_LOW},
                        {'range': [0.12, 0.6], 'color': COLOR_MOD},
                        {'range': [0.6, 1], 'color': COLOR_HIGH}]
                }
            ))
            ...
            st.markdown(f"""<div style="background-color:{bg_color}; padding:40px; border-radius:15px; text-align:center; border: 2px solid #333; margin-top: -20px;">
                <h1 style="color:{t_color}; margin:0; font-size:55px;">{"🔴" if res=="High Risk" else ("🟡" if res=="Moderate Risk" else "🟢")} {res}</h1>
                <p style="color:{t_color}; font-size:26px; font-weight:bold; margin-top:10px;">{advice}</p>
                </div>""", unsafe_allow_html=True)

            current_time = datetime.now(tz_th).strftime("%Y-%m-%d %H:%M:%S")
            new_row = pd.DataFrame([{"Timestamp": current_time, "Case_ID": case_id_input, "Risk_Level": res, "Advice": advice}])
...
with tab2:
    st.markdown(f"""<div style="background-color:#1E1E1E; padding:15px; border-radius:10px; border-left: 5px solid {COLOR_LOW}; margin-bottom:20px;">
            <h3 style="color:white; margin:0;">🚀 AI Performance Metrics</h3>
            <p style="color:{COLOR_LOW}; font-size:24px; font-weight:bold; margin:0;">Sensitivity: 100%</p>
        </div>""", unsafe_allow_html=True)
    ...
    if not df.empty:
        ...
        m2.markdown(f"<div style='background-color:{COLOR_HIGH}; padding:10px; border-radius:5px; text-align:center;'>🔴 High: {len(df[df['Risk_Level']=='High Risk'])}</div>", unsafe_allow_html=True)
        m3.markdown(f"<div style='background-color:{COLOR_MOD}; padding:10px; border-radius:5px; text-align:center; color:black;'>🟡 Mod: {len(df[df['Risk_Level']=='Moderate Risk'])}</div>", unsafe_allow_html=True)
        m4.markdown(f"<div style='background-color:{COLOR_LOW}; padding:10px; border-radius:5px; text-align:center;'>🟢 Low: {len(df[df['Risk_Level']=='Low Risk'])}</div>", unsafe_allow_html=True)
        
        # Charts
        st.markdown("<br>", unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        with c1:
            fig_pie = px.pie(df, names='Risk_Level', color='Risk_Level', color_discrete_map={'High Risk':COLOR_HIGH, 'Moderate Risk':COLOR_MOD, 'Low Risk':COLOR_LOW})
            st.plotly_chart(fig_pie, use_container_width=True)
        with c2:
            df['Date'] = pd.to_datetime(df['Timestamp']).dt.date
            fig_line = px.line(df.groupby('Date').size().reset_index(name='Counts'), x='Date', y='Counts')
            st.plotly_chart(fig_line, use_container_width=True)

        # --- RECENT HISTORY TABLE WITH COLOR HIGHLIGHT ---
        st.subheader("📑 รายการประเมินล่าสุด (Highlight Risk)")
        
        def highlight_risk(val):
            if val == 'High Risk': return f'background-color: {COLOR_HIGH}; color: white; font-weight: bold;'
            elif val == 'Moderate Risk': return f'background-color: {COLOR_MOD}; color: black; font-weight: bold;'
            elif val == 'Low Risk': return f'background-color: {COLOR_LOW}; color: white; font-weight: bold;'
            return ''

        recent_df = df.sort_values(by="Timestamp", ascending=False).head(5)[["Timestamp", "Case_ID", "Risk_Level"]]
        st.table(recent_df.style.applymap(highlight_risk, subset=['Risk_Level']))
    else:
