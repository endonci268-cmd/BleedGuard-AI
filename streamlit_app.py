...
# --- 7. ฟอร์มบันทึกข้อมูล (เพิ่ม ประวัติผ่าตัด และ เคมีบำบัด กลับมา) ---
...
if submit:
    ...
    try:
        ...
        # แบ่งกลุ่มตาม Threshold 0.3 และ 0.5 พร้อมคำแนะนำใหม่
        if clin_risk == "High" or prob >= 0.5:
            ...
            **📋 แผนการพยาบาล (High Risk):**
            ...
            2. **คำแนะนำกลับบ้าน:** งดออกกำลังกายหนัก/ยกของหนัก 7-14 วัน, ทานอาหารอ่อน, หากถ่ายเป็นเลือดหรือปวดท้องรุนแรงให้มา รพ. ทันที
            """
            st_func = st.error
        elif prob >= 0.3:
            ...
            **📋 แผนการพยาบาล (Moderate Risk):**
            1. **ติดตามอาการ:** ในวันที่ 1, 3 และ 7 หลังทำหัตถการ
            2. **คำแนะนำกลับบ้าน:** สังเกตลักษณะอุจจาระและอาการปวดท้อง, เลี่ยงยกของหนัก 3-5 วัน
            """
            st_func = st.warning
        else:
        ...
        st.markdown(f"""
            <div style="background-color: {b_col}; border: 2px solid {color}; padding: 30px; border-radius: 20px; text-align: center;">
                ...
                <p style="color: {color}; font-size: 1.2rem;">
                    <b>AI Probability Score:</b> {prob:.4f} | <b>เวลาประมวลผล:</b> {timestamp}
                </p>
            </div>
        """, unsafe_allow_html=True)
        ...
        st.write("") 
        st_func(advice)
        st.info("💡 ข้อมูลถูกบันทึกเรียบร้อยแล้ว กราฟ Dashboard จะอัปเดตเมื่อเริ่มเซสชันใหม่")
